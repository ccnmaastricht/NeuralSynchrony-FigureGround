...
3. Pre-process human psychophysics data to obtain behavioural Arnold tongues and estimate optimal psychometric parameters for each session.
4. Evaluate assumption of local learning by analysing transfer session results.
5. Explore the parameter space of the model to compare empirically derived parameters to optimal parameter range.
6. Estimate the learning rate of the model using cross-validation.
7. Simulate the learning experiment using the estimated learning rate.
8. Test the model predictions against the human psychophysics data.
9. Generate figures to visualize the results.

Main statistical analyses can be performed in the corresponding notebooks once the snakemake workflow has completed.
